...
function solution(book_time) {
    var answer = [];
    let checkoutArr = []
    book_time.sort().forEach((times) => {
        let checkIn = timeToMin(times[0])
        let checkOut = timeToMin(times[1])
...
- 어떤 알고리즘을 써야하는지 모르겟다.
- 단순히 풀이를 요구한것인지 아니면 특정 알고리즘을 사용해야했는지
- 호텔 방 마다의 큐를 만들었어야 했나라는 생각도 든다.
